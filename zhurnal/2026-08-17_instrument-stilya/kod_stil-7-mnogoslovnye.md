# Канал исполнителя — stil-7-mnogoslovnye (один заход до конца)
...
for l in open(sys.argv[1], encoding="utf-8"):
 ...
 if d.get("type")=="result":
  ...
  for m,v in (d.get("modelUsage") or {}).items():
   print("  %s: вх %s вых %s USD %s" % (m, v.get("inputTokens"), v.get("outputTokens"), v.get("costUSD")))' /tmp/zahod-stil-7-mnogoslovnye.jsonl
```
...
- **ЗОНА (можно менять):** `vypuski/check_oboroty.py` `vypuski/proverki.py` `zhurnal/2026-08-17_instrument-stilya/NOCH` `zhurnal/2026-08-17_instrument-stilya/kod_stil-7-mnogoslovnye.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
- Точка отката: `git add vypuski/check_oboroty.py `vypuski/proverki.py` `zhurnal/2026-08-17_instrument-stilya/NOCH` `zhurnal/2026-08-17_instrument-stilya/kod_stil-7-mnogoslovnye.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
## 2. ЗАДАЧА
Закрыть класс Б — многословные обороты. **Это самое дорогое из непробованного: 13 из 21 оборота на стенде многословны и парами «объект + глагол» не ловятся вовсе.**

Примеры незаконных многословных со стенда: «оценки пошли навстречу друг другу», «здесь сошлось то, что не сошлось в размерности 3», «модель сдвинула у верхней оценки показатель экспоненты». Последний показывает суть трудности: оборот **разорван** вставкой, окно в 4 токена его не видит.

**Что уже есть, не начинай с нуля.** Заход `kod_korpus-wiki.md` 17.08 построил два инструмента:
- `vypuski/oboroty_ngram.json.gz` — 441 492 n-граммы, порог суммарной частоты ≥5. Проверка на 13 эталонных находках: **поймано 6 из 13** по под-n-граммам, причём на самом пороге часть из этих шести не выживет.
- `vypuski/portret_objektov.json.gz` — 300 объектов, разобрано **полным синтаксическим парсером** 575 721 предложение, мера logDice. Контроль сошёлся: «граница + идти» было 8 срабатываний окном, стало 5 синтаксисом.

**Сделать `vypuski/check_oboroty.py`.** Взять синтаксис, который уже посчитан, и свернуть через путь в дереве зависимостей разорванный оборот в смежную пару: «сдвинула … показатель». Это закрывает ровно тот дефект, из-за которого 13 из 21 не ловятся. Литература и меры — `RESERCH-chto-mozhno-postroit.md` §4.3 (PARSEME, разрывные MWE).
⚠ Если синтаксического разбора не хватает — он считался по подкорпусу топ-300 объектов, — скажи прямо и работай на том, что есть. Пересчитывать парсер по всему корпусу в эту ночь **нельзя**: он конкурирует за машину со скачкой.

**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ).**
1. На 13 эталонных многословных находках стенда (`vypuski/pary_idiom.jsonl`) ловится **больше 6** — то есть строго лучше нынешних n-грамм. Не лучше — это законный отрицательный результат, но тогда назови поимённо, какие не ловятся и почему.
2. Число пойманных печатает **команда**, прогоняющая гейт по стенду, а не твоя оценка.
3. Гейт встроен в `vypuski/proverki.py`, фаза `stil`, — как соседние, посмотри и сделай так же.
4. **Контроль на принятом:** прогон по выпускам 02 и 03, принятым владельцем. Числа — в отчёт. Резкий рост означает, что гейт ловит форму, а не брак.
...
Свежий субагент, ДРУГОЙ метод, сверка с бэкапом/`git show`, программно (список потерь ПУСТ; битых ссылок 0). 🔴 **ВЕРИФИКАТОР НУЖЕН.** Утверждение «ловится N из 13» — главное число этой фазы, и проверить его самим гейтом нельзя. Свежий субагент другим методом: берёт 13 эталонных оборотов из `vypuski/pary_idiom.jsonl` ГЛАЗАМИ, прогоняет каждый через гейт по одному и печатает таблицу «оборот → поймано/нет». Ответ обязан кончаться строкой «выдано N позиций из M найденных».
...
```
git --no-optional-locks add -- vypuski/check_oboroty.py vypuski/proverki.py zhurnal/2026-08-17_instrument-stilya/NOCH zhurnal/2026-08-17_instrument-stilya/kod_stil-7-mnogoslovnye.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- vypuski/check_oboroty.py vypuski/proverki.py zhurnal/2026-08-17_instrument-stilya/NOCH zhurnal/2026-08-17_instrument-stilya/kod_stil-7-mnogoslovnye.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `vypuski/check_oboroty.py` `vypuski/proverki.py` `zhurnal/2026-08-17_instrument-stilya/NOCH` `zhurnal/2026-08-17_instrument-stilya/kod_stil-7-mnogoslovnye.md`

- **Г1. Зона доехала в git.** `python3 $MAT/_generator/tools/git_zona.py check --zone vypuski/check_oboroty.py` → ✅; `python3 $MAT/_generator/tools/git_zona.py check --zone vypuski/proverki.py` → ✅; `python3 $MAT/_generator/tools/git_zona.py check --zone zhurnal/2026-08-17_instrument-stilya/NOCH` → ✅; `python3 $MAT/_generator/tools/git_zona.py check --zone zhurnal/2026-08-17_instrument-stilya/kod_stil-7-mnogoslovnye.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
