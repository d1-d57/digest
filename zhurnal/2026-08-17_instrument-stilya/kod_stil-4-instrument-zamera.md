# Канал исполнителя — stil-4-instrument-zamera (один заход до конца)
...
for l in open(sys.argv[1], encoding="utf-8"):
 ...
 if d.get("type")=="result":
  ...
  for m,v in (d.get("modelUsage") or {}).items():
   print("  %s: вх %s вых %s USD %s" % (m, v.get("inputTokens"), v.get("outputTokens"), v.get("costUSD")))' /tmp/zahod-stil-4-instrument-zamera.jsonl
```
...
- **ЗОНА (можно менять):** `vypuski/zamer_elusion.py` `zhurnal/2026-08-17_instrument-stilya/NOCH` `zhurnal/2026-08-17_instrument-stilya/kod_stil-4-instrument-zamera.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
- Точка отката: `git add vypuski/zamer_elusion.py `zhurnal/2026-08-17_instrument-stilya/NOCH` `zhurnal/2026-08-17_instrument-stilya/kod_stil-4-instrument-zamera.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
## 2. ЗАДАЧА
Собрать `vypuski/zamer_elusion.py` — инструмент, которым МЕРЯЕТСЯ полнота, когда эталона нет. Это единственная фаза цепочки, которая делает не «ещё один детектор», а средство узнать, работают ли остальные.

**Зачем.** У нас семь инструментов и **ни одного числа про полноту**. 18.08 замер провалился именно тут: инструмент выдал 378 кандидатов, эталон строить не из чего. Метод взят из практики e-discovery (`RESERCH-chto-mozhno-postroit.md` §5): полноту нельзя посчитать, перебрав всё, но можно ОЦЕНИТЬ, проверив случайный кусок **непоказанного**.

**Что делает скрипт.**
1. Берёт текст и прогоняет по нему гейты стиля. ⚠ **Список гейтов бери НЕ из `vypuski/proverki.py` — проверено 18.08, там его нет:** реестр фазы `stil` перечисляет `check_stil`, `check_bloki` и субагентов-верификаторов, а нужных нам `check_idioma`, `check_fraza`, `check_dlina`, `check_oboroty` в нём не значится, и формат записи — кортежи с именами без модулей. ⇒ **Назови список явно, в одной константе наверху файла, с комментарием, почему не из реестра.** Гейты, которых на момент запуска ещё нет (их делают соседние фазы этой же ночи), пропускай молча и печатай, какие нашлись.
2. Делит все фразы текста на два ведра: **показанные** (хоть один гейт их пометил) и **отсеянные** (не пометил никто).
   🔴 **Сначала определи, что такое «фраза», и это не формальность.** Гейты возвращают разное: `check_idioma` — пары «объект + глагол», `check_fraza` — предложения. Сведи всё к **предложению** как общей единице: пара привязывается к предложению, в котором найдена. Без этого вёдра несравнимы и оба числа получатся бессмысленными. Способ сведения назови в докстроке.
3. Берёт из каждого ведра **случайную** выборку заданного размера (по умолчанию 20 и 20). 🔴 Именно случайную: не «самые спорные», не «топ по частоте». *Причина не эстетическая: на наших объёмах умный отбор примеров для разметки проигрывает случайному, это измерено (`RESERCH §5`, работа «Mind Your Outliers!»), а смещённая выборка делает оба числа несравнимыми между прогонами.*
4. **Перемешивает обе выборки и печатает списком БЕЗ пометок, какая фраза откуда.** Владелец должен судить фразы, а не инструмент. Соответствие «фраза → ведро» пишется в отдельный файл-ключ, который владельцу не показывается.
5. Принимает вердикты владельца обратно (простой формат: номер + да/нет) и считает два числа с доверительным интервалом: долю подтверждённых среди показанных и долю пропущенных среди отсеянных.

**Фиксируй зерно случайности** и печатай его — иначе прогон невоспроизводим.

**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ).**
1. `python3 vypuski/zamer_elusion.py vypuski/2026-08-15-vypusk-04 --vydat` печатает 40 перемешанных фраз без пометок и кладёт файл-ключ рядом. Проверить глазами, что по выданному списку **нельзя** понять, какая фраза из какого ведра.
2. `--svesti <файл с вердиктами>` на **выдуманных** вердиктах (сам проставь для проверки механики) печатает два числа с интервалами и не падает на краевых случаях: все «да», все «нет», часть без ответа.
3. Скрипт не зовёт языковую модель **вообще**: он готовит материал человеку. Проверить грепом, что в нём нет ни одного обращения к API.
...
```
git --no-optional-locks add -- vypuski/zamer_elusion.py zhurnal/2026-08-17_instrument-stilya/NOCH zhurnal/2026-08-17_instrument-stilya/kod_stil-4-instrument-zamera.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- vypuski/zamer_elusion.py zhurnal/2026-08-17_instrument-stilya/NOCH zhurnal/2026-08-17_instrument-stilya/kod_stil-4-instrument-zamera.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `vypuski/zamer_elusion.py` `zhurnal/2026-08-17_instrument-stilya/NOCH` `zhurnal/2026-08-17_instrument-stilya/kod_stil-4-instrument-zamera.md`

- **Г1. Зона доехала в git.** `python3 $MAT/_generator/tools/git_zona.py check --zone vypuski/zamer_elusion.py` → ✅; `python3 $MAT/_generator/tools/git_zona.py check --zone zhurnal/2026-08-17_instrument-stilya/NOCH` → ✅; `python3 $MAT/_generator/tools/git_zona.py check --zone zhurnal/2026-08-17_instrument-stilya/kod_stil-4-instrument-zamera.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
