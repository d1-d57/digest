...
PORYADOK = ['kalibrovka', 'razvedka', 'raskladka', 'karta', 'tekst', 'ponyatnost', 'stil']

FAZA_IMYA = {
    'kalibrovka': 'калибровка — берём ли тему и что в ней объясняется',
...
# У `kalibrovka` и `razvedka` приёмки нет: их артефакты лежат в `docs/`, вне папки
# выпуска, и по имени файла не грепаются отсюда. Строка приёмки в них ЕСТЬ
# (`ПРИНЯТО kalibrovka:` в `docs/dnevnik-intervyu-<тема>.md`), но проверить её
# нечем — тот же класс, что Д23. Молчание здесь объявлено, а не спрятано.
PRIYOMKA = {
...
REESTR = {
    # Заведена 22.08.2026. До неё конвейер начинался с `razvedka`, входом которой
    # значилось «тема выбрана» — а кем и как, не описывал никто. Разговор, в котором
    # это решается, шёл каждый раз по-своему и своей цены не имел: 22.08 он дважды
    # сменил предмет выпуска, и оба раза ДО первого поискового запроса.
    'kalibrovka': [],        # носителей нет, объявленная слепая зона: docs/fazy/kalibrovka.md
