# Канал исполнителя — stil-2-klass-dlinnoy-frazy (один заход до конца)
...
for l in open(sys.argv[1], encoding="utf-8"):
 ...
 if d.get("type")=="result":
  ...
  for m,v in (d.get("modelUsage") or {}).items():
   print("  %s: вх %s вых %s USD %s" % (m, v.get("inputTokens"), v.get("outputTokens"), v.get("costUSD")))' /tmp/zahod-stil-2-klass-dlinnoy-frazy.jsonl
```
...
- **ЗОНА (можно менять):** `vypuski/check_dlina.py` `vypuski/proverki.py` `zhurnal/2026-08-17_instrument-stilya/NOCH` `zhurnal/2026-08-17_instrument-stilya/kod_stil-2-klass-dlinnoy-frazy.md`. Всё вне — **READ-ONLY**: не править, не двигать, не удалять, не рефакторить «заодно».
...
- Точка отката: `git add vypuski/check_dlina.py `vypuski/proverki.py` `zhurnal/2026-08-17_instrument-stilya/NOCH` `zhurnal/2026-08-17_instrument-stilya/kod_stil-2-klass-dlinnoy-frazy.md` → commit (или zip), если зона не чиста в HEAD (не фабрикуй, если чиста).
...
## 2. ЗАДАЧА
Завести НОВЫЙ гейт `vypuski/check_dlina.py` — четвёртый класс брака, которого у нас нет ничем.

**Откуда класс.** Заход `kod_korpus-wiki.md` 17.08 посчитал профиль стиля (`vypuski/profil_stilya.json`) и нашёл единственный признак с большим отклонением:

| признак | kniga | chelovek | nejroset | выпуск-4 |
|---|---|---|---|---|
| средняя длина предложения (токенов) | 15.5 | 16.3 | 14.5 | **26.7** |
| доля страдательных, % | 1.32 | 1.37 | 1.76 | 1.28 |
| доля отглагольных сущ., % | 4.17 | 3.79 | 4.73 | 3.09 |

Выпуск почти **вдвое** длиннее книжного канона по предложению, а страдательный залог и отглагольные — в норме. То есть брак не в словах, а в СТРУКТУРЕ: длинные, перегруженные придаточными фразы.

**Сделать.** Гейт печатает предложения-выбросы: длина сильно выше книжного канона. Порог — **вывести замером, а не назначить**: посчитай распределение длин по слою `kniga` (реестр `vypuski/reestr_korpusa.tsv`, слой `kniga`) и возьми верхний хвост (например, 95-й процентиль), число напечатай в докстроке вместе с тем, как оно получено. Кроме длины полезны придаточные и цепочки родительного падежа — если возьмёшь их, обоснуй числом, а не соображением.

🔴 **ПОРОГ — ПАРАМЕТР, А НЕ КОНСТАНТА. Решение владельца 18.08, дословно:** *«в чём смысл инструмента? Он меряет предложения и сравнивает с каким-то значением корпуса. Так вот это значение просто может быть разным для разных жанров, и это окей. Скорее всего, 15.5 не больше, но может быть где-то меньше — например, для слайдов короткие предложения»*. ⇒ Значение по умолчанию берётся замером по книжному слою, но **обязано задаваться флагом** (`--norma N` или профилем жанра). Зашитое в код число здесь считается браком, а не упрощением: инструмент общий, жанров у нас четыре.

⚠ **Длина сама по себе не брак.** Бывает длинное и хорошее предложение. Гейт **вердиктов не выносит**, он показывает места — как `check_idioma.py` и `check_fraza.py`, и по той же причине. Скопируй у них форму вывода и оговорку.

**Встроить в реестр проверок** `vypuski/proverki.py`, фаза `stil` — посмотри, как там стоят соседние гейты, и сделай так же.

**КРИТЕРИЙ ГОТОВНОСТИ (может ПРОВАЛИТЬСЯ).**
1. `python3 vypuski/check_dlina.py vypuski/2026-08-15-vypusk-04` отрабатывает с `rc=0` и печатает число мест. ⚠ **Числового потолка здесь НЕТ, и это осознанно:** выпуск 4 даёт 26.7 токенов на предложение против 15.5 книжных, то есть при честном пороге мест будет много — это и есть находка, а не поломка. Требование «не раздувать выход» из общего контекста к ЭТОЙ фазе не применяется: подгонять порог, чтобы получилось красивое число, запрещено прямо.
2. Порог получен командой; в отчёт — сама команда и её вывод, а не «взял 95-й процентиль».
3. **Контроль на принятом материале, обязательный:** прогон по выпускам 02 и 03, которые владелец ПРИНЯЛ. Там гейт обязан давать заметно меньше мест, чем на выпуске 4, — иначе он меряет не брак, а длину текста. Числа по всем трём выпускам — в отчёт таблицей. *Этот контроль не формальность: `check_fraza.py` класс B именно на нём и оказался неоткалиброванным.*
...
Свежий субагент, ДРУГОЙ метод, сверка с бэкапом/`git show`, программно (список потерь ПУСТ; битых ссылок 0). Операция не лоссовая — заводится новый файл. Хватает самопроверки критерия §2.
...
```
git --no-optional-locks add -- vypuski/check_dlina.py vypuski/proverki.py zhurnal/2026-08-17_instrument-stilya/NOCH zhurnal/2026-08-17_instrument-stilya/kod_stil-2-klass-dlinnoy-frazy.md                     # вводит НОВЫЕ пути в индекс
git --no-optional-locks commit -m "<зона>: <что сделано>" -- vypuski/check_dlina.py vypuski/proverki.py zhurnal/2026-08-17_instrument-stilya/NOCH zhurnal/2026-08-17_instrument-stilya/kod_stil-2-klass-dlinnoy-frazy.md   # отсекает всё чужое
...
**ЗОНА ГИГИЕНЫ:** `vypuski/check_dlina.py` `vypuski/proverki.py` `zhurnal/2026-08-17_instrument-stilya/NOCH` `zhurnal/2026-08-17_instrument-stilya/kod_stil-2-klass-dlinnoy-frazy.md`

- **Г1. Зона доехала в git.** `python3 $MAT/_generator/tools/git_zona.py check --zone vypuski/check_dlina.py` → ✅; `python3 $MAT/_generator/tools/git_zona.py check --zone vypuski/proverki.py` → ✅; `python3 $MAT/_generator/tools/git_zona.py check --zone zhurnal/2026-08-17_instrument-stilya/NOCH` → ✅; `python3 $MAT/_generator/tools/git_zona.py check --zone zhurnal/2026-08-17_instrument-stilya/kod_stil-2-klass-dlinnoy-frazy.md` → ✅. Красное на любой из команд — отчёт не принимается: приёмка гоняет их все первым ходом.
